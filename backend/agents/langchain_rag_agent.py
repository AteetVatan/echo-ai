...
from backend.constants import REPLY_CACHE_SIMILARITY_THRESHOLD
from backend.knowledge.supabase_compat import (
    CompatibleSupabaseVectorStore as SupabaseVectorStore,
)
from backend.utils import get_settings, get_logger
from backend.db.db_operations import DBOperations, ReplyCacheRecord
from backend.exceptions import DatabaseError, RAGError
...
class LangChainRAGAgent:
    ...
    def _setup_vector_store(self):
        ...
        try:
            from backend.knowledge.self_info_vectorstore import _get_supabase_client
    ...
    # Knowledge base is now managed by backend.knowledge.self_info_vectorstore (persistent, upsert-based).

    def _rebuild_self_info_stores(self):
        """Force rebuild self-info stores (pgvector: truncate + re-embed)."""
        try:
            from backend.knowledge.self_info_vectorstore import (
...
{context}"""
    ...
    # Regex rules + LLM fallback live in backend.agents.query_expansions
    # -----------------------------------------------------------------------
