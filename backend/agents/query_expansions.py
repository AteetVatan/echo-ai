...
QUERY_EXPANSIONS: List[QueryExpansionRule] = [
    # ── Work Experience / Career (BEFORE identity — "about your experience" must not
    #    match the broad "about" rule) ───────────────────────────────
    (
        re.compile(r"\bexperienc", re.I),
    ...
    (
        ...
        "Tell me about Ateet Bahamani — his background, role, and expertise.",
    ),
    (
        re.compile(r"\b(name|full\s*name)\b", re.I),
        "What is Ateet Bahamani's full name?",
    ),
    (
        re.compile(r"\b(bio|biography|profile|summary)\b", re.I),
        "Give me a short bio or introduction of Ateet Bahamani.",
    ),
    (
    ...
    (
        re.compile(
            r"\b(database|supabase|redis|sql|vector\s*db|chroma|faiss|pinecone)\b", re.I
        ),
    ...
    (re.compile(r"\bcertif", re.I), "What certifications does Ateet have?"),
    (
    ...
    # ── Projects / Portfolio ────────────────────────────────────────
    (re.compile(r"\bproject", re.I), "What projects has Ateet worked on?"),
    (
    ...
    (
        re.compile(
            r"\b(hir|interview|recruit|looking\s*for|availab|open\s*to)\b", re.I
        ),
    ...
    (
        re.compile(
            r"\b(ai|artificial\s*intellig|machine\s*learn|ml|deep\s*learn)\b", re.I
        ),
...
_REWRITE_SYSTEM_PROMPT = (
    "You are a query rewriter. The user is visiting Ateet Bahamani's "
...
async def expand_query(user_text: str, llm=None, language: str = "en") -> str:
    ...
    # 1. Try regex FIRST — instant, free, works at ANY word count.
    #    This ensures "tell me about your experience" (5 words) still
    #    matches the \bexperienc rule instead of being skipped.
    regex_result = expand_query_regex(user_text)
    if regex_result:
        logger.info("Query expanded (regex): '%s' → '%s'", user_text, regex_result)
        return regex_result

    # English long queries without regex match: already clear, skip LLM rewrite
